            ...
            DetailView(item: navManager.selectedItem, buttonTab: $buttonTab, switchTab: $switchTab, checkboxTab: $checkboxTab, radioTab: $radioTab, typographyTab: $typographyTab, tagTab: $tagTab, spaceTab: $spaceTab, dividerTab: $dividerTab, badgeTab: $badgeTab, avatarTab: $avatarTab, emptyTab: $emptyTab, spinTab: $spinTab, statisticTab: $statisticTab, alertTab: $alertTab, progressTab: $progressTab, tokenTab: $tokenTab, skeletonTab: $skeletonTab, rateTab: $rateTab, sliderTab: $sliderTab, resultTab: $resultTab, tooltipTab: $tooltipTab, popoverTab: $popoverTab)
                ...
                .toolbar {
                    ToolbarItemGroup(placement: .primaryAction) {
                        ...
                        // Skeleton 页面显示 Tab 切换
                        if navManager.selectedItem == .skeleton {
                             Picker("", selection: $skeletonTab) {
                                 Text(tr("tab.examples")).tag(SkeletonExamplesTab.examples)
                                 Text(tr("tab.api")).tag(SkeletonExamplesTab.api)
                                 Text(tr("tab.token")).tag(SkeletonExamplesTab.token)
                             }
                             .pickerStyle(.segmented)
                             .fixedSize()
                        }
                        ...
                        // Slider 页面显示 Tab 切换
                        if navManager.selectedItem == .slider {
                             Picker("", selection: $sliderTab) {
                                 Text(tr("tab.examples")).tag(SliderExamplesTab.examples)
                                 Text(tr("tab.api")).tag(SliderExamplesTab.api)
                                 Text(tr("tab.token")).tag(SliderExamplesTab.token)
                             }
                             .pickerStyle(.segmented)
                             .fixedSize()
                        }
                        ...
                        // Tooltip 页面显示 Tab 切换
                        if navManager.selectedItem == .tooltip {
                             Picker("", selection: $tooltipTab) {
                                 Text(tr("tab.examples")).tag(TooltipExamplesTab.examples)
                                 Text(tr("tab.api")).tag(TooltipExamplesTab.api)
                                 Text(tr("tab.token")).tag(TooltipExamplesTab.token)
                             }
                             .pickerStyle(.segmented)
                             .fixedSize()
                        }
...
enum NavItem: String, Identifiable {
    ...
    var icon: String {
        ...
        case .slider: return DemoIcons.slider
...
struct DetailView: View {
    ...
    var body: some View {
        Group {
            ...
            case .slider:
                SliderExamples(selectedTab: $sliderTab)
            case .typography:
